...
_SKIP_EXT = {
    ...
    ".pdf", ".zip", ".rar", ".7z", ".gz", ".mp4", ".mp3", ".mov", ".wav",
    ".woff", ".woff2", ".ttf", ".eot"
}
...
def _fetch(url: str, ua: str):
    ...
    text_head = (resp.text[:1000] if isinstance(resp.text, str) else "").lower()
...
def crawl_site(start_url: str, max_pages: int = 10, ua: str = None) -> dict:
    ...
    while queue and len(pages) < max_pages:
        ...
        text = " ".join([t for t in [title or "", h1 or ""] + paragraphs if t]).strip()
