...
def _make_table(data: List[List[Any]], col_widths: List[float]) -> Table:
    # Harden: wrap elke stringcel in P() zodat markup niet geparsed wordt
...
# ---------------- Core helpers (unique, dups, proposals) --------
...
def _propose_copy(
    url: str, title: Optional[str], h1: Optional[str], meta: Optional[str],
    h2_list: Optional[List[str]], h3_list: Optional[List[str]], paras: Optional[List[str]],
    lang: str = "en",
) -> Dict[str, str]:
    ...
    if not _openai_client:
        ...
        for h in (h2_list or [])[:3]: ctx_bits.append(f"H2: {h}")
        for h in (h3_list or [])[:2]: ctx_bits.append(f"H3: {h}")
        for p in (paras or [])[:2]: ctx_bits.append(f"P: {p}")
        if lang == "nl":
            sys = ("Je schrijft compacte webkopie in het Nederlands. "
            ...
            sys = ("You write compact web copy in English. Provide a <title> (≤60 chars), "
                   "a meta description (140–155 chars), an H1 variant (not identical), "
                   "and a short intro (1–2 sentences). Return JSON: title, meta, h1_alt, intro.")
        user = f"URL: {url}\nCONTEXT:\n" + "\n".join(ctx_bits)[:1200]
        try:
            resp = _openai_client.chat.completions.create(
                model=OPENAI_MODEL, temperature=0.2, timeout=OPENAI_TIMEOUT_SEC,
...
def _build_text_findings(crawl: Optional[Dict[str, Any]], site_lang: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for p in pages:
        ...
        if need_title:
            reason = []
            if not title: reason.append("missing" if lang != "nl" else "ontbreekt")
            if title and (len(title) < 30 or len(title) > 65): reason.append("length suboptimal" if lang != "nl" else "lengte suboptimaal")
            if title and title_counts.get(title.lower(), 0) > 1: reason.append("duplicate site-wide" if lang != "nl" else "dubbel op site")
            rows.append({
                "url": url, "field": "title", "problem": ", ".join(reason),
                "current": title or ("(empty)" if lang != "nl" else "(leeg)"),
                "proposed": prop["title"], "html_patch": f"<title>{xml_escape(prop['title'])}</title>", "lang": lang,
            })
        if need_meta:
            reason = []
            if not meta: reason.append("missing" if lang != "nl" else "ontbreekt")
            if meta and (len(meta) < 140 or len(meta) > 160): reason.append("length suboptimal" if lang != "nl" else "lengte suboptimaal")
            if meta and meta_counts.get(meta.lower(), 0) > 1: reason.append("duplicate site-wide" if lang != "nl" else "dubbel op site")
            md = prop["meta"]
            rows.append({
                "url": url, "field": "meta_description", "problem": ", ".join(reason),
                "current": meta or ("(empty)" if lang != "nl" else "(leeg)"),
                "proposed": md, "html_patch": f"<meta name=\"description\" content=\"{_attr_escape(md)}\">", "lang": lang,
            })
        if h1_eq_title:
            h1_alt = prop["h1_alt"]
            rows.append({
                "url": url, "field": "h1", "problem": "H1 duplicates title" if lang != "nl" else "H1 is identiek aan title",
                "current": h1, "proposed": h1_alt, "html_patch": f"<h1>{xml_escape(h1_alt)}</h1>", "lang": lang,
            })
        if low_text:
            intro = prop["intro"]
            rows.append({
                "url": url, "field": "intro_paragraph",
                ...
                "proposed": intro, "html_patch": f"<p>{xml_escape(intro)}</p>", "lang": lang,
            })
    return rows
...
def _build_duplicate_groups(crawl: Optional[Dict[str, Any]], site_lang: Optional[str]):
    ...
    for p in pages:
        title = (p.get("title") or "").strip()
        meta = (p.get("meta_description") or "").strip()
        if title: title_buckets.setdefault(title.lower(), []).append(p)
        if meta: meta_buckets.setdefault(meta.lower(), []).append(p)
    ...
    for _, plist in title_buckets.items():
        ...
        for p in plist:
            ...
            prop = _propose_copy(url, p.get("title"), p.get("h1"), p.get("meta_description"), p.get("h2"), p.get("h3"), p.get("paragraphs"), lang=lang)
...
def _build_seo_findings(crawl: Optional[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for p in pages:
        ...
        title = p.get("title"); h1 = p.get("h1"); md = p.get("meta_description")
        canon = (p.get("canonical") or "").strip()
        noindex = bool(p.get("noindex"))
        og_title = p.get("og_title"); og_desc = p.get("og_description")
...
def _strip_html(s: str) -> str:
    if not s: return ""
    s = html_unescape(s); s = _TAG_RE.sub(" ", s); s = re.sub(r"\s+", " ", s).strip()
    return s

def _trim_words(s: str, limit: int = 80) -> Tuple[str, bool]:
    words = (s or "").split()
    if len(words) <= limit: return " ".join(words), False
...
def _qas_from_jsonld(faq_jsonld: Any) -> List[Dict[str, str]]:
    ...
    def _handle_entity(entity):
        ...
        a_text = ""
        if isinstance(ans, list) and ans: ans = ans[0]
        if isinstance(ans, dict): a_text = ans.get("text") or ans.get("answer") or ""
        q = _strip_html(q); a_text = _strip_html(a_text)
        if q and a_text: out.append({"q": q, "a": a_text})
    ...
    for qa in out:
        ...
        seen.add(key); deduped.append(qa)
    return deduped

def _infer_ptype(url: str, page: Dict[str, Any]) -> str:
    given = (page.get("type") or "").lower().strip()
    if given == "faq": return "faq"
    if page.get("faq_jsonld"): return "faq"
    metrics = page.get("metrics") or {}
    if bool(metrics.get("has_faq_schema")): return "faq"
...
def _aeo_findings_from_faq(faq: Optional[Dict[str, Any]], crawl: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if faq:
        for item in (faq.get("faqs") or []):
            q = (item.get("q") or "").strip(); a = (item.get("a") or "").strip(); src = item.get("source")
            if not src:
                out.append({"url": (faq.get("site") or {}).get("url") or "",
    ...
    if crawl:
        for p in (crawl.get("pages") or []):
            ...
            if any(t == "faqpage" for t in types):
                has_faq = True; break
    if not has_faq:
        out.append({"url": (crawl or {}).get("start_url") or "",
...
def _aeo_findings_from_aeo_job(aeo_concrete: Dict[str, Any]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    if not aeo_concrete: return out
    for page in (aeo_concrete.get("scorecards") or []):
        url = page.get("url") or ""; ptype = (page.get("type") or "other").lower()
        metrics = page.get("metrics") or {}
        ...
        if ptype == "faq":
            if qas_detected == 0:
                out.append({"url": url, "finding": "No Q&A section on page", "severity": "high",
                ...
                out.append({"url": url, "finding": "Too few Q&A on page", "severity": "medium",
            ...
            if not has_faq:
                out.append({"url": url, "finding": "No FAQPage JSON-LD", "severity": "medium",
            ...
            if int(metrics.get("answers_gt_80w") or 0) > 0:
                out.append({"url": url, "finding": "Overlong FAQ answers", "severity": "low",
...
def _aeo_from_job(aeo: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    rows: List[Dict[str, Any]] = []; patches: List[Dict[str, Any]] = []; plan_items: List[Dict[str, Any]] = []; scorecards: List[Dict[str, Any]] = []
    if not aeo: return {"rows": rows, "patches": patches, "plan_items": plan_items, "scorecards": scorecards}
    for page in (aeo.get("pages") or []):
        url = _norm_url(page.get("url") or ""); lang = page.get("lang") or "en"; ptype = _infer_ptype(url, page)
        qas_job = page.get("qas") or []; qas_ld = _qas_from_jsonld(page.get("faq_jsonld") or {}); merged_qas: List[Dict[str, str]] = []
        seen = set()
        for src in (qas_job, qas_ld):
            for qa in src:
                q = _strip_html(qa.get("q") or qa.get("question") or ""); a = _strip_html(qa.get("a") or qa.get("answer") or "")
                if not q or not a: continue
                k = q.strip().lower()
                if k in seen: continue
                seen.add(k); merged_qas.append({"q": q, "a": a})
        ...
        faq_html = page.get("faq_html") or ""; faq_jsonld = page.get("faq_jsonld") or {}
        if ptype == "faq" and faq_html.strip():
            patches.append({"url": url, "field": "faq_html_block", "problem": "Missing short, snippet-ready Q&A on page",
                            "current": "(none or not snippet-ready)", "proposed": "Add FAQ HTML block (3–6 Q&A, answers ≤80 words)",
                            "html_patch": faq_html, "category": "body", "severity": 2, "impact": 5, "effort": 2, "priority": 6.0, "patchable": True})
            task, why = _task_texts_for_content("faq_html_block", lang)
            plan_items.append({"url": url, "task": "Add FAQ section (3–6 Q&A, ≤80 words)" if lang == "en" else "FAQ-blok toevoegen (3–6 Q&A, ≤80 woorden)",
                               ...
                               "category": "content", "field": "faq_html_block", "severity": 2, "impact": 5, "effort": 2, "effort_label": "M",
                               "priority": 6.0, "patchable": True, "html_patch": faq_html, "lang": lang})
        if ptype == "faq" and faq_jsonld:
            html = "<script type=\"application/ld+json\">" + json.dumps(faq_jsonld, ensure_ascii=False) + "</script>"
            patches.append({"url": url, "field": "faq_jsonld", "problem": "No/invalid FAQPage JSON-LD", "current": "(none)",
                            "proposed": "Inject <script type='application/ld+json'>FAQPage…</script> in <head> or <body>",
                            "html_patch": html, "category": "head", "severity": 2, "impact": 4, "effort": 1, "priority": 6.0, "patchable": True})
            plan_items.append({"url": url, "task": "Add FAQPage JSON-LD" if lang == "en" else "FAQPage JSON-LD toevoegen",
                               ...
                               "category": "tag", "field": "faq_jsonld", "severity": 2, "impact": 4, "effort": 1, "effort_label": "S",
                               "priority": 6.0, "patchable": True, "html_patch": html, "lang": lang})
        for cp in (page.get("content_patches") or []):
            field = cp.get("field") or "content"; html_patch = cp.get("html_patch") or ""; category = cp.get("category") or "body"
            severity = int(cp.get("severity") or 2); impact = int(cp.get("impact") or 4); effort = int(cp.get("effort") or 2); priority = float(cp.get("priority") or 5.0)
            patchable = bool(cp.get("patchable", True)); problem = cp.get("problem") or ("Missing content block" if lang == "en" else "Ontbrekend contentblok"); proposed = cp.get("proposed") or None
            patches.append({"url": url, "field": field, "problem": problem, "current": cp.get("current") or "(none)",
                            "proposed": proposed, "html_patch": html_patch, "category": category, "severity": severity,
                            "impact": impact, "effort": effort, "priority": priority, "patchable": patchable})
            task, why = _task_texts_for_content(field, lang)
            plan_items.append({"url": url, "task": task, "why": why, "category": "content", "field": field,
                               "severity": severity, "impact": impact, "effort": effort, "effort_label": {1: "S", 2: "M", 3: "L"}.get(effort, "M"),
                               "priority": priority, "patchable": patchable, "html_patch": html_patch, "lang": lang})
        score = int(page.get("score") or 0); issues = [str(i) for i in (page.get("issues") or [])]
        if ptype == "faq":
        ...
        metrics_in = page.get("metrics") or {}; metrics = dict(metrics_in)
        metrics["qas_detected"] = len(merged_qas); metrics["answers_gt_80w"] = answers_gt_80w
...
def _geo_recommendations(site_meta: Dict[str, Any], crawl: Optional[Dict[str, Any]], schema_job: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if "organization" not in present_types or "website" not in present_types:
        recs.append({"url": home, "finding": "Add/validate Organization + WebSite (entity grounding)", "severity": "high",
    ...
    if "breadcrumblist" not in present_types:
        recs.append({"url": home.rstrip("/") + "/faq", "finding": "Add BreadcrumbList on sections/FAQ", "severity": "medium",
    ...
    if schema_job and schema_job.get("biz_type", "").lower() == "faqpage":
        recs.append({"url": home, "finding": "Broaden schema coverage beyond FAQPage", "severity": "medium",
...
def _collect_proposals_by_url(text_rows: List[Dict[str, Any]]) -> Dict[str, Dict[str, str]]:
    ...
    for r in text_rows:
        if r["field"] not in ("title", "meta_description"): continue
        u = r["url"]; out.setdefault(u, {})
        if r["field"] == "title": out[u]["title"] = r["proposed"]
        if r["field"] == "meta_description": out[u]["meta"] = r["proposed"]
    for u, m in out.items():
        m.setdefault("title", None); m.setdefault("meta", None)
        if m["title"]: m["og_title"] = m["title"]
        if m["meta"]: m["og_desc"] = m["meta"]
    return out
...
def _build_canonical_og_patches(crawl: Optional[Dict[str, Any]], text_rows: List[Dict[str, Any]]):
    ...
    for u, p in by_url.items():
        ...
        if not canon or _norm_url(canon) != preferred:
            patches.append({"url": u, "category": "canonical", "problem": "missing" if not canon else "differs from page URL",
        ...
        if not og_title or not og_desc:
            patches.append({"url": u, "category": "open_graph",
...
def _impact_effort_for_text(field: str) -> Tuple[int, int]:
    f = (field or "").lower()
    if f == "title": return 4, 1
    if f == "meta_description": return 4, 1
    if f == "h1": return 2, 1
    if f == "intro_paragraph": return 3, 2
    return 2, 2

def _impact_effort_for_patch(category: str, problem: str) -> Tuple[int, int]:
    c = (category or "").lower()
    if c == "canonical": return 5, 1
    if c == "open_graph": return 2, 1
    return 2, 2

def _impact_effort_for_finding(finding: str) -> Tuple[int, int]:
    f = (finding or "").lower()
    if f.startswith("http error"): return 4, 3
    if f == "noindex set": return 5, 2
    if f == "orphan/low internal links": return 3, 2
    if f == "missing h1": return 3, 1
    return 2, 2

def _task_texts_for_text(field: str, lang: str) -> Tuple[str, str]:
    nl = lang == "nl"; f = (field or "").lower()
    if f == "title":
...
def _task_texts_for_patch(category: str, lang: str) -> Tuple[str, str]:
    nl = lang == "nl"; c = (category or "").lower()
    if c == "canonical":
...
def _plan_items(text_rows: List[Dict[str, Any]], tag_patches: List[Dict[str, Any]], technical_rows: List[Dict[str, Any]], pages: List[Dict[str, Any]], site_lang: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for r in text_rows:
        ...
        items.append({"url": url, "task": task, "why": why, "field": r["field"], "category": "text",
                      "severity": severity, "impact": impact, "effort": effort, "effort_label": _effort_label(effort),
                      "priority": priority, "patchable": True, "html_patch": r.get("html_patch"), "lang": lang})
    for pch in tag_patches:
        url = pch["url"]; page = by_url.get(url) or {}
        ...
        sev = 2 if (pch["category"] == "canonical" and pch["problem"] == "differs from page URL") else 1
        task, why = _task_texts_for_patch(pch["category"], lang)
        priority = round((impact + sev) / max(1, effort), 2)
        items.append({"url": url, "task": task, "why": why, "field": pch["category"], "category": "tag",
                      "severity": sev, "impact": impact, "effort": effort, "effort_label": _effort_label(effort),
                      "priority": priority, "patchable": True, "html_patch": pch.get("html_patch"), "lang": lang})
    for r in technical_rows:
        if r["finding"] in _PATCHABLE_FINDINGS: continue
        url = r["url"]; page = by_url.get(url) or {}
        lang = _detect_lang([page.get("title") or "", page.get("h1") or ""], site_lang)
        impact, effort = _impact_effort_for_finding(r["finding"]); sev = _sev_num(r["severity"])
        task, why = _task_texts_for_finding(r["finding"], lang); priority = round((impact + sev) / max(1, effort), 2)
        items.append({"url": url, "task": task, "why": why, "field": r["finding"], "category": "technical",
                      "severity": sev, "impact": impact, "effort": effort, "effort_label": _effort_label(effort),
                      "priority": priority, "patchable": False, "html_patch": None, "lang": lang})
    items.sort(key=lambda x: (-x["priority"], x["effort"], x["url"]))
    return items

# ----------------- CSV/JSON export helpers ----------------------
def _csv_base64(rows: List[Dict[str, Any]], columns: List[str]) -> str:
    buff = io.StringIO(); writer = csv.writer(buff); writer.writerow(columns)
    for r in rows:
...
def _patch_rows_for_export(text_rows: List[Dict[str, Any]], tag_patches: List[Dict[str, Any]], site_lang: Optional[str], extra_patches: Optional[List[Dict[str, Any]]] = None) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for r in text_rows:
        impact, effort = _impact_effort_for_text(r["field"]); prob = (r.get("problem") or "").lower()
        if r["field"] == "title" and ("missing" in prob or "duplicate" in prob or "ontbreekt" in prob or "dubbel" in prob): severity = 3
        elif r["field"] == "title": severity = 2
        elif r["field"] == "meta_description" and ("missing" in prob or "ontbreekt" in prob): severity = 2
        elif r["field"] == "meta_description" and ("duplicate" in prob or "dubbel" in prob): severity = 2
        elif r["field"] == "h1": severity = 1
        elif r["field"] == "intro_paragraph": severity = 2
        else: severity = 1
        priority = round((impact + severity) / max(1, effort), 2)
        out.append({"url": r["url"], "field": r["field"], "problem": r.get("problem"),
                    "current": r.get("current"), "proposed": r.get("proposed"), "html_patch": r.get("html_patch"),
                    "category": "head" if r["field"] in ("title", "meta_description") else "body",
                    "severity": {3: "high", 2: "medium", 1: "low"}[severity], "impact": impact, "effort": effort,
                    "priority": priority, "patchable": True})
    for pch in tag_patches:
        impact, effort = _impact_effort_for_patch(pch["category"], pch.get("problem") or "")
        sev = 2 if (pch["category"] == "canonical" and pch.get("problem") == "differs from page URL") else 1
        priority = round((impact + sev) / max(1, effort), 2)
        out.append({"url": pch["url"], "field": pch["category"], "problem": pch.get("problem"),
                    "current": pch.get("current"), "proposed": None, "html_patch": pch.get("html_patch"),
                    "category": "head", "severity": {3: "high", 2: "medium", 1: "low"}[sev],
                    "impact": impact, "effort": effort, "priority": priority, "patchable": True})
    for ap in (extra_patches or []):
        sev_num = 2; sev_lbl = (ap.get("severity") or "medium")
        if isinstance(ap.get("severity"), int): sev_num = ap["severity"]; sev_lbl = {3: "high", 2: "medium", 1: "low"}.get(sev_num, "medium")
        out.append({"url": ap.get("url"), "field": ap.get("field"), "problem": ap.get("problem"),
                    "current": ap.get("current"), "proposed": ap.get("proposed"), "html_patch": ap.get("html_patch"),
                    "category": ap.get("category") or "body", "severity": sev_lbl, "impact": ap.get("impact", 3),
                    "effort": ap.get("effort", 2), "priority": ap.get("priority", 5.0), "patchable": ap.get("patchable", True)})
    return out
...
def _try_llm_summary(site_meta, seo_rows, geo_rows, aeo_rows, aeo_scorecards=None) -> Optional[str]:
    ...
    try:
        sys = ("Je bent een senior SEO/GEO/AEO-auditor. Schrijf 2–4 korte alinea’s in het Nederlands. Wees concreet, kwantificeer waar mogelijk, en prioriteer fixes."
               if is_nl else
               "You are a senior SEO/GEO/AEO auditor. Write 2–4 short paragraphs in English. Be concrete, quantify where possible, and prioritize fixes.")
        payload = {
            "site": {"name": site_meta.get("account_name"), "url": site_meta.get("url"),
                     "language": site_meta.get("language"), "country": site_meta.get("country")},
            "top": {"seo": [r["finding"] for r in seo_rows[:3]],
...
def generate_report(conn, job):
    ...
    elems.append(P(title, "Title"))  # ESCAPED
    elems.append(P(f"Generated: {now}", "Normal"))  # ESCAPED
    elems.append(Spacer(1, 8))
    elems.append(P(exec_summary, "Small"))  # ESCAPED -> voorkomt <link ...> parsing
    ...
    if not text_rows:
        ...
        data = [headers]
        for r in text_rows:
            data.append([_shorten(r["url"], 120), r["field"], r["problem"], _shorten(str(r.get("current", "")), 300), _shorten(str(r.get("proposed", "")), 300)])
        elems.append(_make_table(data, colw))
