...
app = FastAPI(title="Aseon API", version="0.3.1")
...
def job_json(r: dict) -> dict:
    return {
        "id": str(r["id"]), "site_id": str(r["site_id"]), "type": r["type"],
        "payload": r["payload"], "status": r["status"],
        "created_at": iso(r["created_at"]), "started_at": iso(r["started_at"]),
        "finished_at": iso(r["finished_at"]), "error": r["error"]
    }
...
class JobOut(BaseModel):
    id: str; site_id: str; type: str; payload: Optional[Dict[str, Any]] = None
    status: str; created_at: str; started_at: Optional[str] = None
    finished_at: Optional[str] = None; error: Optional[str] = None
