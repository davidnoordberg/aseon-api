...
def finish_job(conn, job_id, ok: bool, output: Optional[dict] = None, err: Optional[Exception] = None):
    with conn.cursor() as cur:
        if ok:
            ...
            log("info", "finish_job_pre_write", job_id=str(job_id), preview=json.dumps(safe_output)[:400])
            cur.execute("""
                UPDATE jobs
...
def build_crawl_context(crawl_output: dict, max_pages: int = 15, max_chars: int = 12000) -> str:
    ...
    slim = {"pages": [], "summary": (crawl_output or {}).get("summary"), "quick_wins": (crawl_output or {}).get("quick_wins")}
...
def run_faq(conn, site_id, payload):
    ...
    data = generate_faqs(conn, site_id, topic=topic, n=n)
    return data
