...
def log(level: str, msg: str, **kwargs):
    print(json.dumps({"ts": datetime.now(timezone.utc).isoformat(), "level": level.upper(), "msg": msg, **kwargs}), flush=True)

def handle_sigterm(signum, frame):
    global running
    log("info", "received_shutdown"); running = False
...
def requeue_stale(conn):
    with conn.cursor() as cur:
        ...
        n = cur.rowcount
        conn.commit()
    if n: log("warn", "requeued_stale_running_jobs", count=n, older_than_min=STALE_MINUTES)
...
def finish_job(conn, job_id, ok: bool, output: Optional[dict] = None, err: Optional[Exception] = None):
    with conn.cursor() as cur:
        if ok:
            ...
            cur.execute("""UPDATE jobs SET status='done', output=%s, finished_at=NOW(), error=NULL WHERE id=%s""", (Json(safe), job_id))
            conn.commit()
        else:
            err_text = (str(err) if err else "Unknown error")[:MAX_ERROR_LEN]
            cur.execute("""UPDATE jobs SET status='failed', finished_at=NOW(), error=%s WHERE id=%s""", (err_text, job_id))
            conn.commit()
    ...
    with conn.cursor() as cur:
        cur.execute("""
         ...
         ORDER BY COALESCE(finished_at, created_at) DESC
            LIMIT 1
        """, (site_id, jtype))
        r = cur.fetchone(); return r["output"] if r else None
...
def build_crawl_context(crawl_output: dict, max_pages: int = 10, max_chars: int = 9000) -> str:
    ...
    for p in pages[:max_pages]:
        slim["pages"].append({
            ...
            "title": p.get("title"), "meta_description": p.get("meta_description"),
            "h1": p.get("h1"), "h2": p.get("h2"), "h3": p.get("h3")
        })
...
def run_crawl(conn, site_id, payload):
    ...
    ua = (payload or {}).get("user_agent") or DEFAULT_UA
...
def run_schema(conn, site_id, payload):
    ...
    if crawl_out:
        ...
        rag_context = explicit_context; context_used = "payload"
    else:
        rag_context = "(no crawl context)"; context_used = "none"

    schema_obj = generate_schema(biz_type=biz_type, site_name=name, site_url=site_url,
                                 language=site_lang, extras=extras, rag_context=rag_context)
    return {"schema": schema_obj, "biz_type": biz_type, "name": name, "url": site_url,
            "language": site_lang, "country": site_country, "_context_used": context_used}
...
DISPATCH_NEEDS_CONN = {"crawl": run_crawl, "schema": run_schema, "faq": run_faq}
DISPATCH_PURE = {"keywords": run_keywords,
                 "report": lambda site_id, payload: {"format": (payload or {}).get("format","markdown"),
...
def process_job(conn, job):
    ...
    if jtype in DISPATCH_NEEDS_CONN: out = DISPATCH_NEEDS_CONN[jtype](conn, site_id, payload)
    elif jtype in DISPATCH_PURE: out = DISPATCH_PURE[jtype](site_id, payload)
    else: raise ValueError(f"Unknown job type: {jtype}")
    log("info", "job_done", id=str(job["id"]), type=jtype); return out
...
def main():
    ...
    log("info", "agent_boot", poll_interval=POLL_INTERVAL_SEC, batch_size=BATCH_SIZE,
        ...
        marker="AGENT_VERSION_V13_LOW_MEM_STALE_REQUEUE", ingest_enabled=INGEST_ENABLED)
    ...
    while running:
        try:
            ...
            log("error", "loop_error", error=str(loop_err)); time.sleep(POLL_INTERVAL_SEC)
    log("info", "agent_exit")
